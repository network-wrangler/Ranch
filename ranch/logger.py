...
__all__ = ["RanchLogger", "setupLogging"]


# for all the Wrangler logging needs!
RanchLogger = logging.getLogger("RanchLogger")

def setupLogging(
    level: int = None, log_filename: str = None, log_to_console: bool = False
):
    """
    Sets up the RanchLogger w.r.t. the debug file location and if logging to console.
    ...
    if level is None:
        RanchLogger.setLevel(logging.DEBUG)
    else:
        RanchLogger.setLevel(level)
    ...
    if log_filename:
        ...
        RanchLogger.addHandler(file_handler)

    if log_to_console:
        console_handler = logging.StreamHandler(sys.stdout)
        console_handler.setLevel(logging.INFO)
        console_handler.setFormatter(FORMAT)
        RanchLogger.addHandler(console_handler)
