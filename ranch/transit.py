...
class Transit(object):
    ...
    @staticmethod
    def load_all_gtfs_feeds(gtfs_dir: str, roadway_network: Roadway, parameters: Dict):
        """
        ...
        for agency in gtfs_agencies_list:
            ...
            if "fare_attributes.txt" in os.listdir(os.path.join(gtfs_dir, agency)):
                fare_attributes_df = pd.read_csv(
                    os.path.join(gtfs_dir, agency, "fare_attributes.txt"),
                    dtype={"fare_id": str},
                )
    ...
    def validate_feed(feed, agency_gtfs_name):
        ...
        trip_stops_df.rename(columns={"stop_id": "stop_pattern"}, inplace=True)
        ...
        if (len(feed.shapes) == 0) and (
            len(feed.trips[feed.trips.shape_id.notnull()]) == 0
        ):  # ACE, CCTA, VINE
            RanchLogger.info("missing shapes.txt for {}".format(agency_gtfs_name))

            group_df = (
                trips_df.groupby(
                    ["agency_raw_name", "route_id", "direction_id", "stop_pattern"]
                )["trip_id"]
                .first()
                .reset_index()
                .drop("trip_id", axis=1)
        ...
        if len(feed.trips[feed.trips.shape_id.isnull()]) > 0:
            RanchLogger.info(
                "missing shape_ids in trips.txt for {}".format(agency_gtfs_name)
            )

            trips_missing_shape_df = trips_df[trips_df.shape_id.isnull()].copy()

            group_df = (
                trips_missing_shape_df.groupby(
                    ["agency_raw_name", "route_id", "direction_id", "stop_pattern"]
                )["trip_id"]
                .first()
                .reset_index()
    ...
    def get_representative_trip_for_route(self, num_most_pattern=1):
        ...
        def get_tod(arrival_hour):
            for period, time_info in model_time_period.items():
                if time_info["start"] < time_info["end"]:
                    if (
                        arrival_hour >= time_info["start"]
                        and arrival_hour < time_info["end"]
                    ):
                        return period
                else:
                    if (
                        arrival_hour >= time_info["start"]
                        or arrival_hour < time_info["end"]
                    ):
                        return period
            return "NA"
            ...
            for period, time_info in model_time_period.items():
                if time_info.get("frequency_start"):
                    if time_info["frequency_start"] < time_info["frequency_end"]:
                        if (
                            arrival_hour >= time_info["frequency_start"]
                            and arrival_hour < time_info["frequency_end"]
                        ):
                            return period
                    else:
                        if (
                            arrival_hour >= time_info["frequency_start"]
                            or arrival_hour < time_info["frequency_end"]
                        ):
                            return period
            return "NA"
        ...
        tod_short_window = [
            key
            for key, value in model_time_period.items()
            if "frequency_start" in value
        ]
        ...
        trip_freq_df.rename(columns={"trip_id": "trip_num_for_shape"}, inplace=True)

        # add the short window time period info to the trip_freq_df
        trip_short_window_df = trip_df[
            [
                "agency_raw_name",
                "route_id",
                "tod",
        ...
        trip_num_df.rename(
            columns={"trip_num_for_shape": "trip_num_total"}, inplace=True
        )
        ...
        trip_freq_df = (
            trip_freq_df.groupby(["agency_raw_name", "route_id", "tod", "direction_id"])
            .head(num_most_pattern)
            .reset_index()
        )
        ...
        trip_stops_df = (
            stop_times_df.groupby(["agency_raw_name", "trip_id"])["stop_sequence"]
            .count()
            .to_frame()
            .reset_index()
        )
    ...
    def snap_stop_to_node(self):
        ...
        link_candidates_for_nodes_df = link_candidates_for_nodes_df[
            (
                (
                    ~link_candidates_for_nodes_df.roadway.isin(
                        ["motorway", "motorway_link", "trunk", "trunk_link"]
                    )
                )
                & (link_candidates_for_nodes_df.assign_group < 100)
                & (
                    (link_candidates_for_nodes_df.drive_access == 1)
                    | (link_candidates_for_nodes_df.drive_access == "1")
                )
            )
            | (
                (link_candidates_for_nodes_df.bus_only == 1)
                | (link_candidates_for_nodes_df.bus_only == "1")
            )
        ].copy()
    ...
    def route_bus_link_osmnx_from_start_to_end(
        ...
        if ft_penalty_suburban:
            ...
            ft_penalty_suburban = (
                self.parameters.transit_routing_parameters_suburban.get("ft_penalty")
            )
        ...
        bus_shapes_df = self.feed.shapes[
            (self.feed.shapes.shape_id.isin(bus_trip_df.shape_id.unique()))
        ].copy()
        ...
        for agency_raw_name in bus_trip_df.agency_raw_name.unique():
            ...
            for trip_id in trip_id_list:
                RanchLogger.info(
                    "\tRouting agency {}, trip {}".format(agency_raw_name, trip_id)
                )
                ...
                if len(shape_df) > 0:
                    ...
                    stop_times_df = pd.merge(
                        stop_times_df,
                        self.feed.stops[
                            ["stop_lat", "stop_lon", "stop_id", "agency_raw_name"]
                        ],
                        how="left",
                ...
                links_within_polygon_gdf["ft_penalty"].fillna(
                    link_penalty["default"], inplace=True
                )
                # bus_only link
                links_within_polygon_gdf["ft_penalty"] = links_within_polygon_gdf[
                    "ft_penalty"
                ...
                RanchLogger.debug(
                    "Routing trip {} from stop {}, osm node {} to stop {} osm node {}".format(
                ...
                if type(path_osm_link_df) == str:
                    self.shortest_path_failed_shape_list.append(
                        "{}_{}".format(agency_raw_name, shape_id)
                    )
                    continue
    ...
    def get_link_path_between_nodes(G, from_node, to_node, weight_field):
        ...
        try:
            node_osmid_list = nx.shortest_path(G, from_node, to_node, weight_field)
        except:
    ...
    def set_good_links(self, shapes, good_links_buffer_radius):
        ...
        good_link_dict = (
            stop_good_link_df.groupby(["agency_raw_name", "shape_id"])[
                "shstReferenceId"
            ]
            .apply(list)
            .to_dict()
    ...
    def links_within_stop_buffer(drive_link_df, stops, buffer_radius):
        ...
        stop_buffer_link_df = stop_buffer_link_df[stop_buffer_link_df.stop_id.notnull()]
    ...
    def links_within_shape_buffer(drive_link_df, shapes, buffer_radius):
        ...
        bus_shapes_gdf = gpd.GeoDataFrame(
            ...
            geometry=gpd.points_from_xy(
                bus_shapes_gdf["shape_pt_lon"], bus_shapes_gdf["shape_pt_lat"]
            ),
            crs=drive_link_df.crs,
        )
        shapes_line_gdf = (
            bus_shapes_gdf.sort_values(by=["shape_pt_sequence"])
            .groupby(["agency_raw_name", "shape_id"])["geometry"]
            .apply(lambda x: LineString(x.tolist()))
        )
        shapes_line_gdf = gpd.GeoDataFrame(
            shapes_line_gdf, geometry="geometry", crs=drive_link_df.crs
        ).reset_index()
    ...
    def route_bus_link_osmnx_between_stops(
        ...
        RanchLogger.info(
            "Routing bus on roadway network from stop to stop with osmnx..."
        )
        ...
        for agency_raw_name in bus_trip_df.agency_raw_name.unique():
            ...
            for trip_id in trip_id_list:
                ...
                RanchLogger.info(
                    "\tRouting agency {}, trip {}, between stops".format(
                ...
                for s in range(len(route_by_stop_df) - 1):
                    ...
                    RanchLogger.debug(
                        "\tRouting trip {} from stop {} node {}, to stop {} node {}".format(
                            trip_stops_df.trip_id.unique(),
    ...
    def match_gtfs_shapes_to_shst(
        self, path: Optional[str] = None, multithread_shst_match: bool = False
    ):
        """
    ...
    def _match_gtfs_shapes_to_shst(
        self, path: str, multithread_shst_match: bool = False
    ):
        """
        ...
        for index, row in lines_from_shapes_df.iterrows():
            ...
            row_gdf.to_file(
                os.path.join(
                    path,
                    "lines_from_shapes_{}_{}.geojson".format(
                        agency_raw_name.replace(" ", ""), shape_id.replace(" ", "")
                    ),
                ),
                driver="GeoJSON",
            )
    ...
    def route_bus_trip(
        ...
        if self.trip_osm_link_df is None:
            ...
            if len(self.trip_osm_link_df) > 0:
                trip_osm_link_gdf.drop("wayId", axis=1).to_file(
                    os.path.join(self.gtfs_dir, "osm_routing.geojson"), driver="GeoJSON"
                )
        ...
        if self.enable_shst_match:
            ...
            if len(trip_shst_link_df) > 0:
                ...
                RanchLogger.info("shst matched 0 bus shapes, 0 bus trips")

            if len(self.trip_osm_link_df) == 0:
                RanchLogger.info(
                    "in the remaining gtfs shapes, shorteset path method matched 0 bus shapes"
                )
        ...
        RanchLogger.info(
            "in the remaining gtfs shapes, shortest path method matched {} bus shapes, {} bus trips".format(
    ...
    def update_bus_stop_node(self):
        ...
        for agency_raw_name in self.bus_trip_link_df.agency_raw_name.unique():
            ...
            for trip_id in self.bus_trip_link_df.trip_id.unique():
                shape_id = self.feed.trips[
                    self.feed.trips.trip_id == trip_id
                ].shape_id.iloc[0]

                if (
                    "{}_{}".format(agency_raw_name, shape_id)
                    in self.shortest_path_failed_shape_list
                ):
                    continue
    ...
    def route_rail_trip(self):
        ...
        if len(rail_shape_df) == 0:
            ...
            shape_id_not_in_shape_data = [
                s
                for s in shape_id_list
                if s not in rail_shape_df.agency_shape_id.tolist()
            ]
    ...
    def create_freq_table(self):
        ...
        freq_df["headway_secs"] = freq_df.apply(
            lambda x: int(
                ...
                * 60
                * 60
                / (x.trip_num_for_shape / x.trip_num_N_most * x.trip_num_total)
            ),
            axis=1,
        )
    ...
    def write_standard_transit(self, path: Optional[str] = None):
        ...
        if len(shape_point_df) == 0:
            RanchLogger.info(
                "None of the GTFS routes are built as standard transit network."
            )
            return
    ...
    def write_rail_links_and_nodes(self, path: Optional[str] = None):
        """
        ...
        if "fromIntersectionId" not in self.unique_rail_links_gdf.columns:
            self.unique_rail_links_gdf = pd.merge(
                ...
                self.unique_rail_nodes_gdf[
                    ["agency_raw_name", "stop_id", "shst_node_id"]
                ].rename(columns={"stop_id": "from_stop_id"}),
                how="left",
                on=["agency_raw_name", "from_stop_id"],
            )
            self.unique_rail_links_gdf.rename(
                columns={"shst_node_id": "fromIntersectionId"}, inplace=True
            )
        if "toIntersectionId" not in self.unique_rail_links_gdf.columns:
            self.unique_rail_links_gdf = pd.merge(
                ...
                self.unique_rail_nodes_gdf[
                    ["agency_raw_name", "stop_id", "shst_node_id"]
                ].rename(columns={"stop_id": "to_stop_id"}),
                how="left",
                on=["agency_raw_name", "to_stop_id"],
            )
            self.unique_rail_links_gdf.rename(
                columns={"shst_node_id": "toIntersectionId"}, inplace=True
            )

        # write out
        self.unique_rail_links_gdf.to_file(os.path.join(path, "rail_links.geojson"))

        self.unique_rail_nodes_gdf.to_file(os.path.join(path, "rail_nodes.geojson"))
